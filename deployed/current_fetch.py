"""
Currents API -- Health & Medical News Fetcher
=============================================
Endpoint : GET https://api.currentsapi.services/v1/search
Auth     : CURRENTS_KEY environment variable (passed as apiKey query param)

------------------------------------------------------------------------
KEY API FACTS
------------------------------------------------------------------------

  keywords=   Exact phrase match in title + description. One phrase
              per request -- no boolean syntax.
  category=   Native filter. We use "health" and "science".
              Full list: GET /v1/available/categories
  page_size   Max 200 (higher than NewsAPI's 100).
  type=       1=news  2=articles  3=discussion  (we use 1)
  start_date  RFC 3339  e.g. 2024-01-01T00:00:00.00+00:00
  end_date    RFC 3339
  Response    {status, news:[{id,title,description,url,image,
               published,author,language,category}]}
  Rate limit  X-RateLimit-Remaining / X-RateLimit-Limit response headers

  Auth note   apiKey as query param is the recommended method per docs.

------------------------------------------------------------------------
REQUEST BUDGET  (20 requests / day limit)
------------------------------------------------------------------------

  Previous design: 1 request per keyword group = 26+ requests / run.
  This exceeded the daily limit on a single invocation.

  Current design: exactly 2 requests per run.
    Request 1 -- category=health, page_size=200  (up to 200 articles)
    Request 2 -- category=science, page_size=200 (up to 200 articles)

  Client-side keyword tagging then scans every returned article's
  title + description against TOPIC_KEYWORDS and assigns one or more
  topic labels.  Articles that match no keyword are kept under the
  label "health_general" or "science_general" so nothing is discarded.

  Budget breakdown per day (assuming one Lambda invocation):
    2  fetch requests
    1  list_categories (optional, only when event has list_categories=true)
    -  remaining 17-18 requests available for other Lambdas / ad-hoc use

------------------------------------------------------------------------
AWS LAMBDA
------------------------------------------------------------------------

  Handler : current_fetch.currentapi_lambda.lambda_handler
  Runtime : Python 3.11  |  Memory: 256 MB  |  Timeout: 30 s

  Environment variables:
    CURRENTS_API        required -- your Currents API key
    CURRENTS_DAYS_BACK  optional -- lookback window in days   (default 7)
    CURRENTS_LANGUAGE   optional -- language code             (default "en")
    CURRENTS_PAGE_SIZE  optional -- articles per request, max 200 (default 200)
...
Usage (local):
    export CURRENTS_API=your_key_here
    pip install requests
    python currents_health_fetcher.py
"""
