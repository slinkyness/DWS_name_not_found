...
param(
    ...
    [string]$Arch = "arm64", # Lambda CPU: x86_64 or arm64 (default)
    [string]$Utils = "lambda_layers\process_lambda_utils.py"
)
...
$PolarsPlatform = if ($Arch -eq "arm64") { "manylinux2014_aarch64" } else { "manylinux2014_x86_64" }
...
# -- Lambda definitions --------------------------------------------------------
$Lambdas = [ordered]@{
    "article"     = @("article_function.py",    "article_process.py")
    "icd_lookup"     = @("icd_metadata.py")
    "trials"    = @("trials_function.py",   "trials_process.py")
    "gho_lookup"      = @("gho_metadata.py")
    "gho"      = @("gho_function.py",     "gho_process.py")
    "ihme_lookup"      = @("ihme_metadata.py",     "ihme_process.py")
    "ihme"      = @("ihme_function.py",     "ihme_process.py")
    "icd_catalogue" = @("icd_catalogue.py")
    "core_process" = @("core_function.py",    "core_process.py")
}
...
# Pass 2: polars separately — pip cannot resolve it with platform constraints
Write-Host "  Installing polars (platfornm: $PolarsPlatform) ..."
& $PythonCmd -m pip install `
    --quiet `
    --target $LayerPkgDir `
    --platform $PolarsPlatform `
    --implementation cp `
    --python-version $PythonVersion `
    --only-binary=:all: `
    --ignore-installed `
...
$ExtraLayers = [ordered]@{
    "vader-layer"    = @{ Packages = @("vaderSentiment");       Platform = $PipPlatform    }
    "pg8000-layer"   = @{ Packages = @("pg8000", "scramp");     Platform = $PipPlatform    }
    "psycopg2-layer" = @{ Packages = @("psycopg2-binary");      Platform = $PolarsPlatform }
}
foreach ($Name in $ExtraLayers.Keys) {
    $Def      = $ExtraLayers[$Name]
    $Packages = $Def.Packages
    $Platform = $Def.Platform
    $PkgDir   = Join-Path (Join-Path $BuildDir $Name) "python"
    $ZipSrc   = Join-Path $BuildDir $Name
    $ZipDst   = Join-Path $DistDir "$Name.zip"

    Write-Host "Building $Name.zip  [$($Packages -join ', '), platform: $Platform] ..."
    ...
    [System.IO.Compression.ZipFile]::CreateFromDirectory($ZipSrc, $ZipDst)
    $MB = [math]::Round((Get-Item $ZipDst).Length / 1MB, 1)
    Write-Host "  -> dist\$Name.zip  ($MB MB)`n"
}
